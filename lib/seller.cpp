	...
	while (key != 54)
	{
		...
		case 49:
		{
			...
			if (option == 121)
			{
				...
				analyse_sql(exp,-1);
			/*	cout << endl << "发布商品成功！" << endl<<"******************************";
				newone->id = "M000";    //获取商品id
				if (goods_head != NULL)
					newone->number = goods_head->number + 1;
				else
					newone->number = 1;
				int num = newone->number, num_of_zero = 3;
				while (num > 0)
				{
					num /= 10;
					newone->id.pop_back();
				...
				newone->on_shelf_time = get_time();
				newone->seller_id = current_user->id;
				newone->status = 1;
				newone->next = goods_head;
				goods_head = newone;
				modify_file(0);*/
